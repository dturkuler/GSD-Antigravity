...
**Example: UI Component (shows key pattern: Antigravity starts server BEFORE checkpoint)**
```xml
<task type="auto">
...
**Use ONLY for:**
- **Authentication gates** - Antigravity tried CLI/API but needs credentials (this is NOT a failure)
...
**Key distinction:** Auth gates are created dynamically when Antigravity encounters auth errors. NOT pre-planned — Antigravity automates first, asks for credentials only when blocked.
</type>
</checkpoint_types>
...
**Auth gate = Antigravity tried CLI/API, got auth error.** Not a failure — a gate requiring human input to unblock.

**Pattern:** Antigravity tries automation → auth error → creates checkpoint:human-action → user authenticates → Antigravity retries → continues
...
### ❌ BAD: Asking human to deploy / ✅ GOOD: Antigravity automates
...
<!-- GOOD: Antigravity deploys, user verifies -->
<task type="auto">
  <name>Deploy to Vercel</name>
...
**Checkpoint priority:**
1. **checkpoint:human-verify** (90%) - Antigravity automated everything, human confirms visual/functional correctness
2. **checkpoint:decision** (9%) - Human makes architectural/technology choices
3. **checkpoint:human-action** (1%) - Truly unavoidable manual steps with no API/CLI

**When NOT to use checkpoints:**
- Things Antigravity can verify programmatically (tests, builds)
- File operations (Antigravity can read files)
