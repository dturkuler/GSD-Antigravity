...
```bash
PHASE_INFO=$(node .agent/skills/gsd/bin/gsd-tools.cjs find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node .agent/skills/gsd/bin/gsd-tools.cjs roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node .agent/skills/gsd/bin/gsd-tools.cjs find-phase "${PHASE}" --raw)
```
