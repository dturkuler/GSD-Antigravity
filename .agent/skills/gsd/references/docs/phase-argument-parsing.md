...
```bash
PHASE_INFO=$(node .agent/skills/gsd/bin/gsd-tools.js find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node .agent/skills/gsd/bin/gsd-tools.js roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node .agent/skills/gsd/bin/gsd-tools.js find-phase "${PHASE}" --raw)
```
