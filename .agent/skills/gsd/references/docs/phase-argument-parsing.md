...
```bash
PHASE_INFO=$(node ./.antigravity/get-shit-done/bin/gsd-tools.js find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ./.antigravity/get-shit-done/bin/gsd-tools.js roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node ./.antigravity/get-shit-done/bin/gsd-tools.js find-phase "${PHASE}" --raw)
```
