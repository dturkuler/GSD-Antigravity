...
```bash
PHASE_INFO=$(node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
