...
**What we're measuring:** How the developer phrases requests, instructions, and feedback -- the structural pattern of their messages to Antigravity.
...
**What we're measuring:** How quickly the developer makes choices when Antigravity presents options, alternatives, or trade-offs.
...
| `delegator` | Defers to Antigravity's recommendation. Trusts the suggestion. |

**Signal patterns:**

1. **Response latency to options** -- How many messages between Antigravity presenting options and developer choosing. Immediate (same message or next) suggests fast-intuitive.
...
4. If developer uses delegation language (> 3 instances) AND rarely overrides Antigravity's choices AND says "sounds good" or "your call" --> `delegator`
...
**What we're measuring:** How the developer approaches problems, errors, and unexpected behavior when working with Antigravity.
...
| `hypothesis-driven` | Investigates independently first, brings specific theories to Antigravity for validation. |
| `collaborative` | Wants to work through the problem step-by-step with Antigravity as a partner. |
...
3. If developer shares their own analysis AND proposes theories ("I think the issue is X because...") AND asks Antigravity to confirm or refute --> `hypothesis-driven`
...
| Rating | Description |
|--------|-------------|
| `pragmatic-fast` | Uses what works, what Antigravity suggests, or what's fastest. Minimal evaluation. |
...
4. **Rejection patterns** -- Does the developer reject Antigravity's suggestions? On what basis (popularity, personal experience, docs quality)?
...
4. If developer names specific libraries unprompted AND overrides Antigravity's suggestions AND expresses strong preferences --> `opinionated`
...
**What we're measuring:** What causes visible frustration, correction, or negative emotional signals in the developer's messages to Antigravity.

**Rating spectrum:**

| Rating | Description |
|--------|-------------|
| `scope-creep` | Frustrated when Antigravity does things that were not asked for. Wants bounded execution. |
| `instruction-adherence` | Frustrated when Antigravity doesn't follow instructions precisely. Values exactness. |
| `verbosity` | Frustrated when Antigravity over-explains or is too wordy. Wants conciseness. |
| `regression` | Frustrated when Antigravity breaks working code while fixing something else. Values stability. |
...
1. If developer corrects Antigravity for doing unrequested work AND uses language like "I only asked for X", "stop adding things", "stick to what I asked" --> `scope-creep`
...
3. If developer asks Antigravity to be shorter AND skips explanations AND expresses annoyance at length ("too much", "just the answer") --> `verbosity`
...
| Rating | Description |
|--------|-------------|
| `self-directed` | Reads code directly, figures things out independently. Asks Antigravity specific questions. |
| `guided` | Asks Antigravity to explain relevant parts. Prefers guided understanding. |
...
2. If developer asks Antigravity to explain concepts AND requests walkthroughs AND prefers Antigravity-mediated understanding --> `guided`
...
Context-dependent splits are resolved during Phase 3 orchestration. The orchestrator presents the split to the developer and asks which pattern represents their general preference. Until resolved, Antigravity uses the dominant pattern with awareness of the context-dependent variation.
