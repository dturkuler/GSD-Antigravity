...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" init progress)
...
- Use `summary-extract` for efficient parsing:
  ```bash
.agent/skills/gsd/bin/gsd-tools.cjs" summary-extract <path> --fields one_liner
  ```
...
.agent/skills/gsd/bin/gsd-tools.cjs" progress bar --raw)
```
...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" audit-uat --raw 2>/dev/null)
```
...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" roadmap get-phase "${CURRENT_PHASE}" 2>/dev/null)
...
- `/gsd:list-phase-assumptions {phase} ${GSD_WS}` — see Antigravity's assumptions
...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" roadmap get-phase "$((Z+1))" 2>/dev/null)
