...
```bash
INIT=$(node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" init progress)
```
...
- `STATE=$(node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" state-snapshot)`
...
- Use `summary-extract` for efficient parsing:
  ```bash
  node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" summary-extract <path> --fields one_liner
  ```
...
PROGRESS_BAR=$(node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" progress bar --raw)
```
