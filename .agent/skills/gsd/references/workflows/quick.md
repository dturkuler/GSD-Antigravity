...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" init quick "$DESCRIPTION")
...
AskUserQuestion(
  ...
  options: [
    ...
    { label: "You decide", description: "Antigravity's discretion" }
  ],
  multiSelect: false
)
```
...
- If user selects "You decide", capture as Antigravity's Discretion in CONTEXT.md
...
- ./ANTIGRAVITY.md (if exists — project-specific guidelines)
...
- ./ANTIGRAVITY.md (if exists — follow project-specific guidelines)
...
- .planning/STATE.md (Project state)
- ./ANTIGRAVITY.md (Project instructions, if exists)
...
**Known Antigravity bug (classifyHandoffIfNeeded):** If executor reports "failed" with error `classifyHandoffIfNeeded is not defined`, this is a Antigravity runtime bug — not a real failure. Check if summary file exists and git log shows commits. If so, treat as successful.
