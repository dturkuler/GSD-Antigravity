...
```bash
INIT=$(node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" init quick "$DESCRIPTION")
```
...
```bash
node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" commit "docs(quick-${next_num}): ${DESCRIPTION}" --files ${file_list}
```
