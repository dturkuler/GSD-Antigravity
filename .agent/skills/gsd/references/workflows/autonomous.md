...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" init milestone-op)
...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" config-get workflow.skip_discuss 2>/dev/null || echo "false")
```
...
All implementation choices are at Antigravity's discretion — discuss phase was skipped per user setting. Use ROADMAP phase goal, success criteria, and codebase conventions to guide decisions.
...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" commit "docs(${PADDED_PHASE}): auto-generated context (discuss skipped)" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" roadmap get-phase ${PHASE_NUM} 2>/dev/null)
...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" config-get workflow.ui_review 2>/dev/null || echo "true")
```
