...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node .agent/skills/gsd/bin/gsd-tools.cjs frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node .agent/skills/gsd/bin/gsd-tools.cjs roadmap get-phase "${phase_number}" --raw)
```
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node .agent/skills/gsd/bin/gsd-tools.cjs verify artifacts "$plan")
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node .agent/skills/gsd/bin/gsd-tools.cjs verify key-links "$plan")
