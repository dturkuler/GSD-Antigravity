...
```bash
TRANSITION=$(node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" progress bar --raw)
```
...
```bash
node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" config-set workflow.auto_advance false
```
