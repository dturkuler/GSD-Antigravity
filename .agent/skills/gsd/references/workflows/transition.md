...
```bash
TRANSITION=$(node ".agent/skills/gsd/bin/gsd-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node ".agent/skills/gsd/bin/gsd-tools.cjs" progress bar --raw)
```
...
```bash
node ".agent/skills/gsd/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false
```
