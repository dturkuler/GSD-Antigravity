...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" init map-codebase)
...
**Runtimes with Task tool:** Antigravity, Cursor, OpenCode (native subagent support via `Task` or `task`)
...
3. Consider adding sensitive files to Antigravity "Deny" permissions
...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" commit "docs: map existing codebase" --files .planning/codebase/*.md
```
