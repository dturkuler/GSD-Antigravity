...
.agent/skills/gsd/bin/gsd-tools.cjs" agent-skills gsd-integration-checker 2>/dev/null)
```
...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" resolve-model gsd-integration-checker --raw)
```
...
.agent/skills/gsd/bin/gsd-tools.cjs" phases list
```
...
.agent/skills/gsd/bin/gsd-tools.cjs" find-phase 01 --raw)
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
.agent/skills/gsd/bin/gsd-tools.cjs" summary-extract "$summary" --fields requirements_completed --pick requirements_completed
done
```
...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" config-get workflow.nyquist_validation --raw 2>/dev/null)
```
