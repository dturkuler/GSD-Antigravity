...
```bash
INIT=$(node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" init milestone-op)
```
...
```bash
integration_checker_model=$(node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" resolve-model gsd-integration-checker --raw)
```
...
node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" phases list
```
...
PHASE_INFO=$(node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" find-phase 01 --raw)
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" summary-extract "$summary" --fields requirements_completed | jq -r '.requirements_completed'
done
```
