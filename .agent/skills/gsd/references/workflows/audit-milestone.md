...
```bash
INIT=$(node ".agent/skills/gsd/bin/gsd-tools.cjs" init milestone-op)
...
```bash
integration_checker_model=$(node ".agent/skills/gsd/bin/gsd-tools.cjs" resolve-model gsd-integration-checker --raw)
```
...
node ".agent/skills/gsd/bin/gsd-tools.cjs" phases list
```
...
PHASE_INFO=$(node ".agent/skills/gsd/bin/gsd-tools.cjs" find-phase 01 --raw)
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node ".agent/skills/gsd/bin/gsd-tools.cjs" summary-extract "$summary" --fields requirements_completed | jq -r '.requirements_completed'
done
```
...
```bash
NYQUIST_CONFIG=$(node ".agent/skills/gsd/bin/gsd-tools.cjs" config get workflow.nyquist_validation --raw 2>/dev/null)
```
