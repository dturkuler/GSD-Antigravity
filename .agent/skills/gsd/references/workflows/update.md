...
RUNTIME_DIRS=( "antigravity:.antigravity" "opencode:.config/opencode" "opencode:.opencode" "gemini:.gemini" "kilo:.config/kilo" "kilo:.kilo" "codex:.codex" )
ENV_RUNTIME_DIRS=()
...
# If runtime is still unknown, infer from runtime env vars; fallback to antigravity.
if [ -z "$PREFERRED_RUNTIME" ]; then
...
if [ -n "$PREFERRED_CONFIG_DIR" ] && { [ -f "$PREFERRED_CONFIG_DIR/get-shit-done/VERSION" ] || [ -f "$PREFERRED_CONFIG_DIR/get-shit-done/workflows/update.md" ]; }; then
  ...
  echo "$INSTALL_SCOPE"
  echo "${PREFERRED_RUNTIME:-antigravity}"
  exit 0
fi
...
- Line 3 = target runtime (`antigravity`, `opencode`, `gemini`, `kilo`, or `codex`)
- If scope is `UNKNOWN`, proceed to install step using `--antigravity --global` fallback.
...
global: `C:/projects/GSD-Antigravity/.antigravity/`, `~/.config/opencode/`, `~/.opencode/`, `~/.gemini/`, `~/.config/kilo/`, or `~/.codex/`
local: `./.antigravity/`, `./.config/opencode/`, `./.opencode/`, `./.gemini/`, `./.kilo/`, or `./.codex/`)

Your custom files in other locations are preserved:
- Custom commands not in `commands/gsd/` ✓
- Custom agents not prefixed with `gsd-` ✓
- Custom hooks ✓
- Your ANTIGRAVITY.md files ✓
...
**If UNKNOWN install:**
```bash
npx -y get-shit-done-cc@latest --antigravity --global
```
