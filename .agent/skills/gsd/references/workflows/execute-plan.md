...
```bash
node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" phases list --type summaries --raw
...
See `./.antigravity/get-shit-done/references/tdd.md` for structure.
</tdd_plan_execution>
...
See ./.antigravity/get-shit-done/references/checkpoints.md for details.
</step>
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `assets/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `assets/templates/summary.md`.
...
node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>
...
```bash
node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
