...
command -v antigravity >/dev/null 2>&1 && echo "antigravity:available" || echo "antigravity:missing"
command -v codex >/dev/null 2>&1 && echo "codex:available" || echo "codex:missing"
```

Parse flags from `$ARGUMENTS`:
- `--gemini` → include Gemini
- `--antigravity` → include Antigravity
- `--codex` → include Codex
...
Commit:
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" commit "docs: cross-AI review for phase {N}" --files {phase_dir}/{padded_phase}-REVIEWS.md
```
</step>
