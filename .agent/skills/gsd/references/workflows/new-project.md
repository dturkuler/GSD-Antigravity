...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" init new-project)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
.agent/skills/gsd/bin/gsd-tools.cjs" agent-skills gsd-project-researcher 2>/dev/null)
.agent/skills/gsd/bin/gsd-tools.cjs" agent-skills gsd-synthesizer 2>/dev/null)
.agent/skills/gsd/bin/gsd-tools.cjs" agent-skills gsd-roadmapper 2>/dev/null)
```
...
else RUNTIME="antigravity"; fi
```

Set the instruction file variable:
```bash
if [ "$RUNTIME" = "codex" ]; then INSTRUCTION_FILE="AGENTS.md"; else INSTRUCTION_FILE="ANTIGRAVITY.md"; fi
```
...
```bash
mkdir -p .planning
.agent/skills/gsd/bin/gsd-tools.cjs" config-new-project '{"mode":"yolo","granularity":"[selected]","parallelization":true|false,"commit_docs":true|false,"model_profile":"quality|balanced|budget|inherit","workflow":{"research":true|false,"plan_check":true|false,"verifier":true|false,"nyquist_validation":true|false,"auto_advance":true}}'
```
...
```bash
mkdir -p .planning
.agent/skills/gsd/bin/gsd-tools.cjs" commit "chore: add project config" --files .planning/config.json
```
...
```bash
mkdir -p .planning
.agent/skills/gsd/bin/gsd-tools.cjs" commit "docs: initialize project" --files .planning/PROJECT.md
```
...
assets/templates/research-project/STACK.md
</output>
...
assets/templates/research-project/FEATURES.md
</output>
...
assets/templates/research-project/ARCHITECTURE.md
</output>
...
assets/templates/research-project/PITFALLS.md
</output>
...
assets/templates/research-project/SUMMARY.md
...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" commit "docs: define v1 requirements" --files .planning/REQUIREMENTS.md
```
...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" generate-antigravity-md --output "$INSTRUCTION_FILE"
```
...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" commit "docs: create roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md "$INSTRUCTION_FILE"
```
...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" roadmap get-phase 1 2>/dev/null)
...
- `.planning/ROADMAP.md`
- `.planning/STATE.md`
- `$INSTRUCTION_FILE` (`AGENTS.md` for Codex, `ANTIGRAVITY.md` for all other runtimes)
...
- [ ] `$INSTRUCTION_FILE` generated with GSD workflow guidance (AGENTS.md for Codex, ANTIGRAVITY.md otherwise)
