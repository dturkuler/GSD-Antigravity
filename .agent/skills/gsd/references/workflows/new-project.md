...
SPIKE_SKILL=$(ls ./.antigravity/skills/spike-findings-*/SKILL.md 2>/dev/null | head -1)

# Check for sketch findings skill (project-local)
SKETCH_SKILL=$(ls ./.antigravity/skills/sketch-findings-*/SKILL.md 2>/dev/null | head -1)
...
**If TEXT_MODE is active** (non-Antigravity runtimes): display a numbered list and ask the user to type the numbers of settings they want to change (comma-separated). Parse the response and proceed.
...
**Otherwise** (Antigravity runtime with AskUserQuestion): use multiSelect:
...
```bash
gsd-sdk query generate-antigravity-md --output "$INSTRUCTION_FILE"
```
