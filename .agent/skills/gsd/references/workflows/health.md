...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" validate health $REPAIR_FLAG
```
...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" validate health
```
...
<stale_task_cleanup>
**Windows-specific:** Check for stale Antigravity task directories that accumulate on crash/freeze.
...
TASKS_DIR="C:/projects/GSD-Antigravity/.antigravity/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$(find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null | wc -l)
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in C:/projects/GSD-Antigravity/.antigravity/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf C:/projects/GSD-Antigravity/.antigravity/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
