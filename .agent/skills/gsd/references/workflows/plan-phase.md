...
```bash
PHASE_INFO=$(node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
5. Commit:
```bash
node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_DESC=$(node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" | jq -r '.section')
```
...
```
Task(
  prompt="First, read references/agents/gsd-phase-researcher.md for your role and instructions.\n\n" + research_prompt,
...
**If found:**
1. Read validation template from `assets/templates/VALIDATION.md`
...
```bash
node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" commit-docs "docs(phase-${PHASE}): add validation strategy"
```
...
```
Task(
  prompt="First, read references/agents/gsd-planner.md for your role and instructions.\n\n" + filled_prompt,
  subagent_type="general-purpose",
  model="{planner_model}",
...
Task(
  prompt="
    ...
    @references/docs/model-profile-resolution.md
    </execution_context>
