...
```bash
INIT=$(node ./.antigravity/get-shit-done/bin/gsd-tools.js init plan-phase "$PHASE" --include state,roadmap,requirements,context,research,verification,uat)
```
...
```bash
PHASE_DESC=$(node ./.antigravity/get-shit-done/bin/gsd-tools.js roadmap get-phase "${PHASE}" | jq -r '.section')
...
STATE_SNAP=$(node ./.antigravity/get-shit-done/bin/gsd-tools.js state-snapshot)
...
- **Antigravity's Discretion** = Freedom areas — research options, recommend
...
- **Antigravity's Discretion** = Freedom — make implementation choices
...
- **Antigravity's Discretion** = Freedom areas — plans can choose approach
