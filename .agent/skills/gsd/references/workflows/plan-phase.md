...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" init plan-phase "$PHASE")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
.agent/skills/gsd/bin/gsd-tools.cjs" agent-skills gsd-researcher 2>/dev/null)
.agent/skills/gsd/bin/gsd-tools.cjs" agent-skills gsd-planner 2>/dev/null)
.agent/skills/gsd/bin/gsd-tools.cjs" agent-skills gsd-checker 2>/dev/null)
...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
3. Parse the PRD content and generate CONTEXT.md. The orchestrator should:
   ...
   - Identify any areas the PRD doesn't cover and mark as "Antigravity's Discretion"
...
5. Commit:
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick section)
```
...
**Project skills:** Check .antigravity/skills/ or .agents/skills/ directory (if either exists) — read SKILL.md files, research should account for project skill patterns
</additional_context>
...
**If found:**
assets/templates/VALIDATION.md`
...
.agent/skills/gsd/bin/gsd-tools.cjs" config-get workflow.security_asvs_level --raw 2>/dev/null || echo "1")
.agent/skills/gsd/bin/gsd-tools.cjs" config-get workflow.security_block_on --raw 2>/dev/null || echo "high")
```
...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
.agent/skills/gsd/bin/gsd-tools.cjs" config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" 2>/dev/null)
...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick section 2>/dev/null)
```
...
.agent/skills/gsd/bin/gsd-tools.cjs" config-set workflow.nyquist_validation false`
...
**Project instructions:** Read ./ANTIGRAVITY.md if exists — verify plans honor project guidelines
**Project skills:** Check .antigravity/skills/ or .agents/skills/ directory (if either exists) — verify plans account for project skill rules
</verification_context>
...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" state planned-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
.agent/skills/gsd/bin/gsd-tools.cjs" config-set workflow._auto_chain_active true
fi
```
...
stdio deadlocks with MCP servers — see Antigravity issue anthropics/antigravity-code#28126):
...
3. **Clean up stale task directories:**
   ```powershell
   # Remove stale subagent task dirs (Antigravity never cleans these on crash)
   Remove-Item -Recurse -Force "$env:USERPROFILE\.antigravity\tasks\*" -ErrorAction SilentlyContinue
   ```
4. **Reduce MCP server count:** Temporarily disable non-essential MCP servers in settings.json
5. **Retry:** Restart Antigravity and run `/gsd-plan-phase` again
