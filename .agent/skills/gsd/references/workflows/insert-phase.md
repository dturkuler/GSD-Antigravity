...
```bash
INIT=$(node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" init phase-op "${after_phase}")
```
...
```bash
RESULT=$(node "$HOME.agent/skills/gsd/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
