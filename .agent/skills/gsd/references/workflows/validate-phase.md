...
```bash
AUDITOR_MODEL=$(node ".agent/skills/gsd/bin/gsd-tools.cjs" resolve-model gsd-nyquist-auditor --raw)
NYQUIST_CFG=$(node ".agent/skills/gsd/bin/gsd-tools.cjs" config get workflow.nyquist_validation --raw)
```
...
```
Task(
  prompt="Read references/agents/gsd-nyquist-auditor.md for instructions.\n\n" +
...
**State B (create):**
1. Read template from `assets/templates/VALIDATION.md`
...
node ".agent/skills/gsd/bin/gsd-tools.cjs" commit-docs "docs(phase-${PHASE}): add/update validation strategy"
```
