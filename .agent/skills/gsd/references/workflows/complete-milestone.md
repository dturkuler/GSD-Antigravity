...
```bash
ROADMAP=$(node .agent/skills/gsd/bin/gsd-tools.cjs roadmap analyze)
```
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node .agent/skills/gsd/bin/gsd-tools.cjs summary-extract "$summary" --fields one_liner | jq -r '.one_liner'
done
```
...
```bash
ARCHIVE=$(node .agent/skills/gsd/bin/gsd-tools.cjs milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
**Commit:**
```bash
node .agent/skills/gsd/bin/gsd-tools.cjs commit "docs: update retrospective for v${VERSION}" --files .planning/RETROSPECTIVE.md
```
...
```bash
INIT=$(node .agent/skills/gsd/bin/gsd-tools.cjs init execute-phase "1")
```
...
```bash
node .agent/skills/gsd/bin/gsd-tools.cjs commit "chore: complete v[X.Y] milestone" --files .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/MILESTONES.md .planning/PROJECT.md .planning/STATE.md
```
```
