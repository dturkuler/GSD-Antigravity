...
**Project instructions:** Read `./ANTIGRAVITY.md` if it exists in the working directory. Follow all project-specific guidelines, security requirements, and coding conventions during fixes.
...
Read `./ANTIGRAVITY.md` and check for `.antigravity/skills/` or `.agents/skills/` (as described in `<project_context>`).
</step>
...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" commit \
...
_Fixed: {timestamp}_
_Fixer: Antigravity (gsd-code-fixer)_
_Iteration: {N}_
```
...
**DO respect ANTIGRAVITY.md project conventions** during fixes. If project requires specific patterns (e.g., no `any` types, specific error handling), apply them.
...
- [ ] Project conventions from ANTIGRAVITY.md respected during fixes
