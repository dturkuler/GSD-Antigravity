...
| `## Antigravity's Discretion` | Freedom areas — planner can choose approach, don't flag. |
...
**Process:**
1. Parse CONTEXT.md sections: Decisions, Antigravity's Discretion, Deferred Ideas
...
## Dimension 10: ANTIGRAVITY.md Compliance

**Question:** Do plans respect project-specific conventions, constraints, and requirements from ANTIGRAVITY.md?

**Process:**
1. Read `./ANTIGRAVITY.md` in the working directory (already loaded in `<project_context>`)
...
4. Flag plans that introduce patterns ANTIGRAVITY.md explicitly forbids
5. Flag plans that skip steps ANTIGRAVITY.md explicitly requires (e.g., required linting, specific test frameworks, commit conventions)

**Red flags:**
- Plan uses a library/pattern ANTIGRAVITY.md explicitly forbids
- Plan skips a required step (e.g., ANTIGRAVITY.md says "always run X before Y" but plan omits X)
- Plan introduces code style that contradicts ANTIGRAVITY.md conventions
- Plan creates files in locations that violate ANTIGRAVITY.md's architectural constraints
- Plan ignores security requirements documented in ANTIGRAVITY.md

**Skip condition:** If no `./ANTIGRAVITY.md` exists in the working directory, output: "Dimension 10: SKIPPED (no ANTIGRAVITY.md found)" and move on.
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
.agent/skills/gsd/bin/gsd-tools.cjs" verify plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" frontmatter get "$PLAN_PATH" --field must_haves)
```
...
```bash
.agent/skills/gsd/bin/gsd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
- [ ] ANTIGRAVITY.md compliance checked (plans respect project conventions)
