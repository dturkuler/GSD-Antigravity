 ...
 *
 * In multi-repo workspaces, Antigravity may open inside a sub-repo (e.g. `backend/`)
...
function loadConfig(cwd) {
  ...
  sub_repos: [],
  resolve_model_ids: false, // false: return alias as-is | true: map to full Antigravity model ID | "omit": return '' (runtime uses its default)
...
function resolveModelInternal(cwd, agentType) {
  ...
  // default model. For non-Antigravity runtimes (OpenCode, Codex, etc.) that don't recognize
  // Antigravity aliases (opus/sonnet/haiku/inherit). Set automatically during install. See #1156.
  ...
  // resolve_model_ids: true — map alias to full Antigravity model ID
