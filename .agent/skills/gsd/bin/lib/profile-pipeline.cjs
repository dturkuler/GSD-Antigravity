 ...
 *
 * Reads Antigravity session history (read-only) to extract user messages
...
function getSessionsDir(overridePath) {
  const dir = overridePath || path.join(os.homedir(), '.antigravity', 'projects');
...
async function cmdScanSessions(overridePath, options, raw) {
  ...
  if (!sessionsDir) {
  const searchedPath = overridePath || '~/.antigravity/projects';
  error(`No Antigravity sessions found at ${searchedPath}.${overridePath ? '' : ' Is Antigravity installed?'}`);
  }
