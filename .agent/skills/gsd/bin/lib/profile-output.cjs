 ...
 *   - generate-antigravity-profile: Developer Profile section in ANTIGRAVITY.md
 *   - generate-antigravity-md: full ANTIGRAVITY.md with managed sections
 */
...
const PROFILING_QUESTIONS = [
  ...
  context: 'Think about the last few times you asked Antigravity to build or change something. How did you frame the request?',
  question: 'When you ask Antigravity to build something, how much context do you typically provide?',
  options: [
  ...
  context: 'Think about times when Antigravity presented you with multiple options -- like choosing a library, picking an architecture, or selecting an approach.',
  question: 'When Antigravity presents you with options, how do you typically decide?',
  options: [
    ...
    { label: 'Let Antigravity recommend -- I generally trust the suggestion', value: 'd', rating: 'delegator' },
  ],
  },
  {
  dimension: 'explanation_depth',
  header: 'Explanation Preferences',
  context: 'Think about when Antigravity explains code it wrote or an approach it took. How much detail feels right?',
  question: 'When Antigravity explains something, how much detail do you want?',
  options: [
  ...
  context: 'Think about the last few times something broke in your code. How did you approach it with Antigravity?',
  question: 'When something breaks, how do you typically approach debugging with Antigravity?',
  options: [
    { label: 'Paste the error and say "fix it" -- get it working fast', value: 'a', rating: 'fix-first' },
    { label: 'Share the error plus context, ask Antigravity to diagnose what went wrong', value: 'b', rating: 'diagnostic' },
    { label: 'Investigate myself first, then ask Antigravity about my specific theories', value: 'c', rating: 'hypothesis-driven' },
  ...
  options: [
    { label: 'Use whatever Antigravity suggests -- speed matters more than the perfect choice', value: 'a', rating: 'pragmatic-fast' },
  ...
  options: [
    ...
    { label: 'Ask Antigravity to explain the relevant parts to me', value: 'b', rating: 'guided' },
...
// ─── ANTIGRAVITY.md Section Generators ─────────────────────────────────────────────
 ...
 * (name + description) for each. Returns a table summary for ANTIGRAVITY.md so
...
function cmdWriteProfile(cwd, options, raw) {
  ...
  if (!outputPath) {
  outputPath = path.join(os.homedir(), '.antigravity', 'get-shit-done', 'USER-PROFILE.md');
...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  let outputPath = options.output;
  if (!outputPath) {
  outputPath = path.join(os.homedir(), '.antigravity', 'commands', 'gsd', 'dev-preferences.md');
  } else if (!path.isAbsolute(outputPath)) {
  outputPath = path.join(cwd, outputPath);
  }
...
function cmdGenerateAntigravityProfile(cwd, options, raw) {
  ...
  let targetPath;
  if (options.global) {
  targetPath = path.join(os.homedir(), '.antigravity', 'ANTIGRAVITY.md');
