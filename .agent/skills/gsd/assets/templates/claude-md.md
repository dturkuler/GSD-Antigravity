...
Template for project-root `ANTIGRAVITY.md` — auto-generated by `gsd-tools generate-antigravity-md`.

Contains 7 marker-bounded sections. Each section is independently updatable.
The `generate-antigravity-md` subcommand manages 6 sections (project, stack, conventions, architecture, skills, workflow enforcement).
The profile section is managed exclusively by `generate-antigravity-profile`.
...
**Fallback text:**
```
No project skills found. Add skills to any of: `.antigravity/skills/`, `.agents/skills/`, `.cursor/skills/`, or `.github/skills/` with a `SKILL.md` index file.
```

**Discovery behavior:**
- Scans `.antigravity/skills/`, `.agents/skills/`, `.cursor/skills/`, `.github/skills/` for subdirectories containing `SKILL.md`
...
**Note:** This section is NOT managed by `generate-antigravity-md`. It is managed exclusively
by `generate-antigravity-profile`. The placeholder above is only used when creating a new
ANTIGRAVITY.md file and no profile section exists yet.
...
When a source file is missing, fallback text provides Antigravity-actionable guidance:
- Guides Antigravity's behavior in the absence of data
- Not placeholder ads or "missing" notices
- Each fallback tells Antigravity what to do, not just what's absent
